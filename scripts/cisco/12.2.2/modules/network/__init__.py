                            ...
                            try:
                                ...
                                if vlan_id in network_lookup:
                                    ...
                                    if api_func(fabric_name, network_name, switch_name, [interface_name]):
                                        print(f"✅ {operation.capitalize()}ed {network_name} {'to' if operation == 'attach' else 'from'} {interface_name} (ACCESS)")
                                        processed_count += 1
                                    else:
                            ...
                            try:
                                ...
                                for vlan_id in vlan_ids:
                                    if vlan_id in network_lookup:
                                        network_name = network_lookup[vlan_id]
                                        if api_func(fabric_name, network_name, switch_name, [interface_name]):
                                            print(f"✅ {operation.capitalize()}ed {network_name} {'to' if operation == 'attach' else 'from'} {interface_name} (TRUNK)")
                                            processed_count += 1
                                        else:
                                            print(f"❌ Failed to {operation} {network_name} {'to' if operation == 'attach' else 'from'} {interface_name}")
                                            success = False
